...
Vector3 PlayFishingStateBase::Floating(FloatingPattern floatingPattern)
{
    ...
    case CameraPattern:
        m_floating_t += 0.08f;
        m_floating.y = cos(m_floating_t * 0.2f) * 12.0f;
        m_floating.x = cos(m_floating_t * 0.5f) * 2.0f;
        break;
    case ModelPattern:
        m_floating_t += 0.05f;
        m_floating.y = cos(m_floating_t * 0.9f) * 0.3f;
        m_floating.z = cos(m_floating_t * 0.7f) * 0.5f;
        m_floating.x = cos(m_floating_t * 0.3f) * 0.2f;
        break;
    default:
        break;
    }
